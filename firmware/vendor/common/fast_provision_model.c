...
void mesh_fast_prov_proc()
{
	...
	if(((mesh_fast_prov_sts_get() == FAST_PROV_COMPLETE) && clock_time_exceed(fast_prov.start_tick, fast_prov.delay*1000)) ||
		...
		light_res_sw_save_t gy_light_res_sw_save[LIGHT_CNT] = {{{{0}}}};
		light_res_sw_trans_t gy_light_res_sw[LIGHT_CNT] ;
		u8 gy_i_t;
		for(gy_i_t = 0; gy_i_t < LIGHT_CNT; gy_i_t++)
		{
			memcpy((u8*)(&gy_light_res_sw_save[gy_i_t]), (u8*)(&light_res_sw_save[gy_i_t]), sizeof(light_res_sw_save_t));
			memcpy((u8*)(&gy_light_res_sw[gy_i_t]), (u8*)(&light_res_sw[gy_i_t]), sizeof(light_res_sw_trans_t));
		}
		mesh_revert_network();	
		for(gy_i_t = 0; gy_i_t < LIGHT_CNT; gy_i_t++)
		{
			memcpy((u8*)(&light_res_sw_save[gy_i_t]), (u8*)(&gy_light_res_sw_save[gy_i_t]), sizeof(light_res_sw_save_t));
			memcpy((u8*)(&light_res_sw[gy_i_t]), (u8*)(&gy_light_res_sw[gy_i_t]), sizeof(light_res_sw_trans_t));
			light_par_save(1);
		}
	...
	switch(fast_prov.rcv_op){
		...
		case VD_MESH_ADDR_SET:
			...
			//else if(fast_prov.cur_sts != FAST_PROV_IDLE)
			//{
			//	fast_prov.start_tick = clock_time()|1;
			//}
			break;
...
{
	gy_fast_provision_reset_handle();
