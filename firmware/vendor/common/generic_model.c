...
int mesh_tx_cmd_g_onoff_st(u8 idx, u16 ele_adr, u16 dst_adr, u8 *uuid, model_common_t *pub_md, u16 op_rsp)
{
    ...
    //rsp.present_onoff = gy_public_info.current_light_onoff_state;
    //rsp.target_onoff = gy_public_info.current_light_onoff_state;
...
{
	if(par[0] != 0x00 && par[0] != 0x01)
	{
		gy_FCT_opcode_handle(par, par_len, cb_par);
		return 0;
	}
	if(!is_provision_success())
	{
		...
		{
			if(gy_fct_opcode_handle_info.test_result == 0)
			{
				return 0;
			}
			else
			{
				//if(gy_fct_opcode_handle_info.current_test_step == gy_test_step_2 && par[0] == 0x00)
				//{
				//	gy_fct_opcode_handle_info.current_test_step = gy_test_step_3;
				//}
				//else if(gy_fct_opcode_handle_info.current_test_step == gy_test_step_3 && par[0] == 0x01)
				//{
				//	gy_fct_opcode_handle_info.current_test_step = gy_test_step_4;
				//}
			}
		}
	}
	...
	if(par[0] == 1)//开灯
	{
		...
		{
			gy_public_info.current_light_onoff_state = par[0];
		}
	}
	*/
	if(par[0] == 0)//关灯
	{
		if(!(cb_par->retransaction))
		{
			if(gy_i2c_info.sensor_type == gy_i2c_sensor_2)//如果是微波人感
			{
				gy_sensor_freeze_time_count_down = 20;//关灯之后锁死2秒（20*100ms）
			}
			else
			{
...
//			gy_public_info.standby_recorvery_flag=0;
	...
	else if(par[0] == 1)//开灯
	{
		if(!(cb_par->retransaction))
		{
			if(gy_public_info.standby_recorvery_flag)
			{
				gy_light_self_dim_value_set(gy_public_info.dim_value_before_into_standby);
				return err;
			}
...
int mesh_cmd_sig_g_level_set(u8 *par, int par_len, mesh_cb_fun_par_t *cb_par)
{
    ...
    //if(par_len == 8)//Delta_Level(4B) + TID(1B) + Transition_Time(1B) + Delay(1B) + gy_idx(1B)
    //{
    //	if(cb_par->model_idx != par[7])
    //	{
    //		return err;
    //	}
    //}
