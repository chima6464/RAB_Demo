...
void light_pwm_init()
{
    ...
    foreach(i, LIGHT_CNT){
        ...
        #endif
        {
        	//gy_public_info.current_light_onoff_state = 0;
        	if(gy_flash_info.fast_power_onoff_flag == 2)
        	{
...
void proc_led(void)
{
	...
	if (led_event_pending)
	{
		...
		if(gy_vendor_model_info.emergency_event_flag == gy_emergency_event_set)
		{
			led_count = GY_EMERGENCY_EVENT_LED_BLINK_COUNT_MAX;
			gy_vendor_model_info.emergency_event_flag = gy_emergency_event_execute;
		}
		else
		{
			led_count = (led_event_pending>>16) & 0xff;
	...
	if( 1 ){
		if( (u32)(clock_time() - led_tick) >= (led_is_on ? led_ton : led_toff) ){
			...
			if (led_is_on)
			{
				led_no++;
				//led_dbg++;
				if (led_no - 1 == led_count  || gy_vendor_model_info.emergency_event_flag == gy_emergency_event_forced_stop)
				{
					led_count = led_no = 0;
					if(gy_public_info.light_blink_finish_state == gy_light_blink_finish_provision)
					{
						u8 gy_set_light_state_factory[] = {0x20,0x4e,0x00};
						mesh_cmd_sig_lightness_set(gy_set_light_state_factory, sizeof(gy_set_light_state_factory), NULL);
			...
			if( led_off || led_on  ){
                if (led_sel & BIT(0))
                {
                	...
                	{
                		led_no = 0;//���޸ĵĹ��ܣ�����emergency����֮��û��ʱ�����ƣ�ֻҪû���յ�ָֹͣ���һֱ��˸
                	}
...
_USER_CAN_REDEFINE_ void rf_link_light_event_callback (u8 status)
{
	...
	if(status == LGT_CMD_SET_MESH_INFO){
	    ...
	    #else
	    gy_public_info.light_blink_finish_state = gy_light_blink_finish_provision;
...
{
	gy_flash_factory_reset();
	gy_flash_new_factory_reset();
	gy_flash_fix_factory_reset();
