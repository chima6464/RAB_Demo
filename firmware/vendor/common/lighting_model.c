...
s16 get_target_level_by_op(s16 target_level, s32 level, u16 op, int light_idx, int st_trans_type)
{
	...
	if (is_level_delta_set_op(op))
	{
		...
		s16 gy_target_level_value_t = get_val_with_check_range(target_level + level, range_s16.min, range_s16.max, st_trans_type);
		if (gy_target_level_value_t < (range_s16.min + 655))
		{
			gy_target_level_value_t = (range_s16.min + 655);
		}
		return gy_target_level_value_t;
	}
	...
	{ // ((G_LEVEL_SET == op) || (G_LEVEL_SET_NOACK == op))
		return level;
	}
}
...
void model_pub_check_set_bind_all(st_pub_list_t *pub_list, mesh_cb_fun_par_t *cb_par, int linear)
{
	...
	if (pub_list->st[ST_TRANS_LIGHTNESS])
	{
		...
		if (PTS_TEST_EN)
		{ // PTS LLN-BV09C: request publish onoff status, even though onoff not change.
			model_pub_check_set_onoff(pub_list->st[ST_TRANS_PUB_ONOFF], light_idx, p_res->id);
		}
...
int g_level_set(u8 *par, int par_len, u16 op, int idx, bool4 retransaction, int st_trans_type, int force, st_pub_list_t *pub_list)
{
	...
	if (is_level_delta_set_op(set_trans.op))
	{
		...
		if (par_len >= sizeof(mesh_cmd_g_level_delta_t))
		{ // existed transit_t and delay.
		...
		/*
		if(gy_public_info.current_light_onoff_state == 0 && p_set->level <= 0)
		{
			return 0;
		}
		else
		{
			gy_public_info.current_light_onoff_state = 1;
		}
		*/
	}
	else
	{
		...
		if (par_len >= sizeof(mesh_cmd_g_level_set_t))
		{ // existed transit_t and delay.
			if (!is_valid_transition_step(p_set->transit_t))
			{
				return -1;
	...
	// gy_sensor_occupied_time_handle_ctl_light(st_trans_type);
...
{
	if (!is_provision_success() && gy_fct_opcode_handle_info.current_test_step != gy_test_step_idle && gy_fct_opcode_handle_info.test_result == 0)
	{
		return 0;
	}
	// TODO: Macro here
	if (is_provision_success() && (light_res_sw_save[0].level[0].onoff == 0) && (cb_par->adr_src != ele_adr_primary) && (cb_par->adr_src != 0))
	{
		return 0;
	}
