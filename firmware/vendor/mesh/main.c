...
{
	if(gy_public_info.light_ssr_state < 2)
	{
		if(lc_first_cross_zero_flag==0)
		{
			gy_relay_onoff_set(gy_public_info.light_ssr_state);
			gy_public_info.light_ssr_state = 2;  //»Ö¸´
		}
		else
		{
			lc_first_cross_zero_flag=0;
		}
	}
	return;
}
