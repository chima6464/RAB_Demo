...
int app_event_handler (u32 h, u8 *p, int n)
{
	...
	//------------ ADV packet --------------------------------------------
		if (subcode == HCI_SUB_EVT_LE_ADVERTISING_REPORT)	// ADV packet
		{
			...
			gy_fct_opcode_handle_info.rcv_rssi = pa->data[pa->len];
			#if MD_REMOTE_PROV
...
void user_init()
{
		...
		gy_flash_init();
		gy_flash_new_init();
		gy_flash_energy_init();
	//	gy_flash_will_not_erase_init();
		gy_relay_init();
	...
	rf_set_power_level_index (/*my_rf_power_index*/RF_POWER_P10p46dBm);
